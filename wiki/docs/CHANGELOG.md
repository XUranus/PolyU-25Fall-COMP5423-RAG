...
- **Fixed broken f-strings in `rag_pipeline.py`**: `logger.info("start init new generator: ...")` and `logger.info("new generator: ... loaded.")` were missing `f` prefix, causing literal placeholder to be printed instead of the actual model name (`rag_pipeline.py`)
